...
:Namespace direct_tcpip
    ...
    ∇ direct_tcpip methods;sock;session;auth_list;listensock;forwardsock;fhost;fport;meth;channel;poller;ps;buf;r;again
        ...
        :For meth :In methods
            ...
            :Trap #.SSH.SSH_ERR
                ...
                :Case 'password'
                    ...
                    →success
                :Case 'publickey'
                    session.Userauth_Password username keyfile1 keyfile2 password
                    →success
                :Else
                    ⎕←'Unknown method.'
                :EndSelect
        ...
        listensock.setsockopt_int #.Sock.Cnst.SOL_SOCKET #.Sock.Cnst.REUSEADDR
        ...
        fhost fport←forwardsock.ListenConnect
        fport←⍎fport ⍝ convert to number 

        ⎕←'Forwarding connection from here to remote ',remote_desthost,':',remote_destport

        channel←session.Channel_Direct_TCPIP remote_desthost remote_destport shost sport
